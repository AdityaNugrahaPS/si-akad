                ...
                <table id="dataTable" class="table table-striped table-bordered text-nowrap w-100">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="text-start">Tingkat</th>
                            <th scope="col" class="text-start">Nama Kelas</th>
                    ...
                    <tbody>
                        @foreach ($waliKelas as $wali)
                            <tr>
                                <td class="text-start">{{ $wali->guru->nama_lengkap }}</td> <!-- Guru through relation -->
                                <td class="text-start">{{ $wali->tingkat }}</td>
                                <td class="text-start">{{ $wali->kelas->nama_kelas }}</td> <!-- Class through relation -->
                                <td class="text-start">
                                    <a class="btn btn-primary" href="{{ route('wali-kelas.edit', $wali->id) }}">
                                        <i class="fa fa-edit"></i> Edit
                                    </a>
                                    <form class="d-inline" action="{{ route('wali-kelas.delete', $wali->id) }}"
                                        method="post">
                                        @csrf
                                        @method('delete')
