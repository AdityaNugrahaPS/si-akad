...
@section('content')
  ...
  <div class="card">
    <div class="card-header">
      <h3 class="mb-4"><i class="fa fa-users"></i> Edit Rombel</h3>
      <form action="{{ route('rombel.update', $rombel->id) }}" method="POST">
        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control @error('tahun_pelajaran') is-invalid @enderror" id="tahun_pelajaran" name="tahun_pelajaran" value="{{ old('tahun_pelajaran') ?? $rombel->tahun_pelajaran }}">
          ...
          <div class="col-md-6 mb-3">
            ...
            <select name="kelas_id" class="form-select @error('kelas_id') is-invalid @enderror">
              @foreach ($kelas as $kls)
                <option value="{{ $kls->id }}" {{ (old('kelas_id') ?? $rombel->kelas_id) == $kls->id ? "selected" : "" }}>{{ $kls->nama_kelas }}</option>
              @endforeach
            </select>
            @error('kelas_id')
              <div class="invalid-feedback text-danger">
          ...
          <div class="col-md-6 mb-3">
            ...
            <input type="text" class="form-control @error('nama_siswa') is-invalid @enderror" id="nama_siswa" name="nama_siswa" value="{{ old('nama_siswa') ?? $rombel->nama_siswa }}">
            @error('nama_siswa')
              <div class="invalid-feedback text-danger">
                {{ $message }}
              </div>
