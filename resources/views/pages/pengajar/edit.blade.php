...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="mb-4"><i class="fa fa-edit"></i> Edit Pengajar</h3>
            <form action="{{ route('pengajar.update', $pengajar->id) }}" method="POST">
                ...
                <div class="row">
                    ...
                    <!-- Menggunakan guru_id sebagai foreign key -->
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="guru_id" id="guru_id" class="form-select @error('guru_id') is-invalid @enderror">
                            ...
                            @foreach ($gurus as $guru)
                                <option value="{{ $guru->id }}"
                                    {{ (old('guru_id') ?? $pengajar->guru_id) == $guru->id ? 'selected' : '' }}>
                                    {{ $guru->nama_lengkap }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <!-- Mengganti input text Nama Mata Pelajaran menjadi dropdown -->
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="mata_pelajaran_id" id="mata_pelajaran_id"
                            ...
                            @foreach ($mataPelajarans as $mapel)
                                <option value="{{ $mapel->id }}"
                                    {{ (old('mata_pelajaran_id') ?? $pengajar->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>
                        ...
                        <select name="kelas_id" id="kelas_id" class="form-select @error('kelas_id') is-invalid @enderror">
                            ...
                            @foreach ($kelas as $item)
                                <option value="{{ $item->id }}"
                                    {{ (old('kelas_id') ?? $pengajar->kelas_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->nama_kelas }}
                                </option>
                            @endforeach
                        </select>
